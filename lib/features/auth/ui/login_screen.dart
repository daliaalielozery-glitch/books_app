...
class _LoginScreenState extends State<LoginScreen> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      // SafeArea تحمي العناصر من التداخل مع النوتش أو الساعة
      body: SafeArea(
        child: SingleChildScrollView( // تحل مشكلة الـ Overflow عند ظهور الكيبورد
          ...
          child: Padding(
            ...
            child: Column(
              ...
              children: [
                ...
                // إضافة InkWell لجعل "نسيت كلمة السر" قابلة للضغط
                Align(
                ...
                // شكل الـ OR Divider المحسن
                Row(
                  children: [
                    Expanded(child: Divider(thickness: 1.h, color: AppColors.greyColor)),
                    Padding(
                      padding: EdgeInsets.symmetric(horizontal: 10.w),
                      child: Text(LocaleKeys.or.tr(), style: TextStyle(color: AppColors.greyColor)),
                    ),
                    Expanded(child: Divider(thickness: 1.h, color: AppColors.greyColor)),
                  ],
                ),
  ...
  Future<void> login() async {
    try {
      ...
      // هنا تقدر تضيف Navigator للصفحة الرئيسية بعد نجاح اللوجن
    } catch (e) {
      debugPrint("Login Error: $e");
      // يفضل تظهر SnackBar للمستخدم توضح إن فيه خطأ
    }
  }
}