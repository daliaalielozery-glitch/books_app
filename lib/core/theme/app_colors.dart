import 'package:flutter/material.dart';
class AppColors{
  //static to call it when I write .
  //const because its a constant color

    static const  Color primaryColor =Color(0xffBFA054);
    static const  Color whitetxt =Color(0xffFFFFFF);
    static const  Color borderColor =Color(0xffE8ECF4);
    static const  Color greyColor =Color(0xffF7F8F9);
    static const  Color darkgrey =Color(0xff8391A1);
    static const  Color productColor =Color(0xffF5EFE1);



}
